...
int m2_Signal_hall   = 3;  // Hall sensor input (use interrupt-capable pin if available)
...
void wheelMoveForwardLeft() {
  analogWrite(m1_VR_speed, left_speed);
...
void wheelMoveForwardRight() {
  analogWrite(m2_VR_speed, right_speed);
  digitalWrite(m2_ZF_Direction, LOW);   // 🔄 Inverted
  digitalWrite(m2_EL_Start_Stop, HIGH);
}

void wheelMoveBackwardRight() {
  analogWrite(m2_VR_speed, right_speed);
  digitalWrite(m2_ZF_Direction, HIGH);  // 🔄 Inverted
  digitalWrite(m2_EL_Start_Stop, HIGH);
}
